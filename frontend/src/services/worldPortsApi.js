...
// Mapping des noms de pays pour gérer les différences français/anglais
...
export const worldPortsService = {
  getPortsByCountryIso2: async (iso2) => {
    ...
    const portsMatched = []
    Object.entries(REAL_PORTS_DATABASE).forEach(([countryName, ports]) => {
      ;(ports || []).forEach((port) => {
        if (String(port?.countryCode || '').toUpperCase() === wanted) {
          portsMatched.push({
            id: port.id,
            name: port.name,
            nomPort: port.name,
    ...
    return {
      hasPorts: false,
      ports: [],
      message: 'Aucun port disponible pour ce pays',
    }
  },
  ...
  /**
   * Récupère tous les ports disponibles avec leurs frais UNCTAD
   */
  getAllPorts: () => {
    const allPorts = []
    let portIndex = 1
    Object.entries(REAL_PORTS_DATABASE).forEach(([country, ports]) => {
      ports.forEach(port => {
        allPorts.push({
          ...
          country: country,
          pays: country,
          countryCode: port.countryCode || 'XX',
          currency: port.currency || 'USD',
          capacity: port.capacity,
          lat: port.coordinates?.lat || port.lat,
          lon: port.coordinates?.lon || port.lon,
          latitude: port.coordinates?.lat || port.lat,
          longitude: port.coordinates?.lon || port.lon,
          coordinates: port.coordinates || { lat: port.lat, lon: port.lon },
          fees: port.fees || {
            THC: 250,
            portDues: 0.15,
            pilotage: 450,
